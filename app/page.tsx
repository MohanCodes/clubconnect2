...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    if (window.location.hash === "#clubs") {
      const el = document.getElementById("clubs");
      if (el) {
        // Wait a tick to ensure Masonry content has rendered
        setTimeout(() => {
        ...
        <div className="flex flex-col items-center w-full -mt-[18vh]">
          ...
          {filteredClubs.length === 0 && (
            <BlurFade delay={0} inView className='flex justify-center'>
              <div className="rounded-xl p-9 transition-shadow duration-300 bg-tilegrey lg:w-1/2 ">
