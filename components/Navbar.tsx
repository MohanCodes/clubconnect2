    ...
    <>
      ...
      <nav className="sticky top-0 z-50 bg-cblack">
        <div className="flex items-center justify-between p-6">
          <Link href="/" className="flex items-center md:space-x-2 transition duration-300 hover:opacity-70">
            <Image
              src="/circles.svg"
              alt="Logo"
              width={60}
            ...
            <div className="flex justify-end items-end">
              <span className="text-2xl md:text-3xl font-bold text-white nav-text-below-300">
                <span className="text-grey">MN</span>
                <span className="text-white">Club</span>
                <span className="text-azul">Connect</span>
              </span>
              <div className="-ml-10 hidden transition duration-300 ease-in-out hover:-rotate-15">
          ...
          <button
            ...
            className="lg:hidden text-white nav-image-below-300 transition duration-300 hover:opacity-70"
          ...
          >
            {isOpen && (
              <button
                ...
                className="absolute top-8 right-5 text-white text-md transition duration-300 hover:opacity-70"
            ...
            {user && (
              <div className={`flex ${isOpen ? 'flex-col space-y-4' : 'hidden lg:flex lg:flex-row lg:space-x-10'} text-center`}>
                <Link href="/dashboard" className="text-white transition duration-300 hover:opacity-70">Dashboard</Link>
              </div>
            )}
            ...
            {user ? (
              <button
                ...
                className="rounded-full text-white md:pr-0 lg:pr-4 transition duration-300 hover:opacity-70"
              >
                Settings
              </button>
            ) : (
              <div className={`flex ${isOpen ? 'flex-col space-y-4' : 'hidden lg:flex lg:flex-row lg:space-x-10'} items-center`}>
                <Link href="/signin" className="text-white transition duration-300 hover:opacity-70">Sign In</Link>
                <Link
                  href="/signup"
                  className="px-6 py-2.5 rounded-full bg-azul text-white transition duration-300 hover:opacity-70"
                >
                  Signup
                </Link>
              </div>
